...
#9. Obtener el nombre de los de los jefes que tengan su departamento situado en la ciudad de “Ciudad Real”
...
SELECT nombre,SUM(sal_emp+comision_emp+500) AS Total_Pagar FROM empleados WHERE id_depto = 3000 GROUP BY nombre ORDER BY nombre ASC;
...
SELECT nombre,sal_emp,sal_emp+comision_emp AS salario_total FROM empleados WHERE comision_emp > 1000;

#15. Obtener un listado similar al anterior, pero de aquellos empleados que NO tienen comisión.
SELECT nombre,sal_emp,sal_emp+comision_emp AS salario_total,comision_emp FROM empleados WHERE comision_emp = 0;
SELECT comision_emp FROM empleados;
...
SELECT DISTINCT id_depto,ROUND(AVG(sal_emp)) as Promedio FROM empleados GROUP BY id_depto;
select round(avg(sal_emp)),id_depto from empleados group by id_depto;

#Consultas con Having
#25. Hallar los departamentos que tienen más de tres empleados. Mostrar el número de empleados de esos departamentos.
SELECT COUNT(nombre_depto) AS cantidadDeEmpleados,nombre_depto FROM departamentos INNER JOIN empleados on departamentos.id_depto = empleados.id_depto GROUP BY nombre_depto HAVING COUNT(nombre_depto) > 3;
#26. Hallar los departamentos que no tienen empleados
SELECT COUNT(nombre_depto) AS cantidadDeEmpleados,nombre_depto FROM departamentos INNER JOIN empleados on departamentos.id_depto = empleados.id_depto GROUP BY nombre_depto HAVING COUNT(nombre_depto) = 0;
...
SELECT 
    nombre, nombre_jefe_depto, nombre_depto
FROM
    empleados
        INNER JOIN
...
SELECT nombre,sal_emp,id_depto,(SELECT ROUND(AVG(sal_emp)) FROM empleados) AS promedioEmpresa FROM empleados WHERE sal_emp >= (SELECT AVG(sal_emp)FROM empleados) GROUP BY id_depto,nombre,sal_emp ORDER BY id_depto;
select * from empleados where sal_emp >= (select avg(sal_emp) from empleados) order by nombre;